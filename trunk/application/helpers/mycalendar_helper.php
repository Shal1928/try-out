...
function week_number($year = null) {
    if (empty($year))
        $year = date("Y");

    $retval = array();
    $awal_bulan = strtotime("$year-01-01");
    for ($i = 1; $i <= 12; $i++) {
        $temp = next_month($awal_bulan);
        if ($i == 12)
            $temp = next_day($temp, -1);

        $sub = date("W", $awal_bulan) < 52 ? date("W", $awal_bulan) : 0;
        $retval[] = date("W", $temp) - $sub;
        $awal_bulan = $temp;
    }
    if (array_sum($retval) > 52)
        $retval[0]--;
    if (($as = array_sum($retval) < 52))
        $retval[11] += (52 - $as);
    return $retval;
}
